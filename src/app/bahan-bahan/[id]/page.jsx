...
const DetailBahans = ({ params: { id } }) => {
  const judul = decodeURIComponent(id);
  ...
  const currentBahans = bahans.slice(indexFirst, indexLast);

  const goToNextPage = () => setCurrentPage(currentPage + 1);
  const goToPreviousPage = () => setCurrentPage(currentPage - 1);

  const totalPages = Math.ceil(bahans.length / page);
      ...
      <div className='py-10 px-4 md:px-16 xl:px-36'>
        <div>
          <h1 className='text-black text-xl md:text-xl font-extrabold xl:text-xl'>
            Masakan dari Bahan-bahan <span className='text-secondary'>{judul}</span>
          </h1>
          <div className='grid grid-cols-2 sm:grid-cols-3 gap-5 mt-3 md:mt-5'>
            {currentBahans.map((kategori) => (
        ...
        <div className='flex flex-col lg:flex-row gap-5  items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 mt-5'>
          <p className='text-sm text-gray-700'>
            Showing <span className='font-medium'>{indexFirst + 1}</span> to <span className='font-medium'>{currentPage === totalPages ? bahans.length : indexLast}</span> of{" "}
          ...
          <div className='flex flex-1 justify-between sm:hidden'>
            ...
            <button
              ...
              disabled={currentBahans.length < page}

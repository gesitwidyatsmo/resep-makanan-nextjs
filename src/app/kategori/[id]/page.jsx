...
const DetailKategories = ({ params: { id } }) => {
  ...
  const currentKategoris = kategoris.slice(indexFirst, indexLast);

  const goToNextPage = () => setCurrentPage(currentPage + 1);
  const goToPreviousPage = () => setCurrentPage(currentPage - 1);

  const totalPages = Math.ceil(kategoris.length / page);
  ...
  return (
    <div>
      ...
      <div className='py-10 px-4 md:px-16 xl:px-36'>
        <div>
          <h1 className='text-black text-xl md:text-xl font-extrabold xl:text-xl'>Kategori {id}</h1>
        ...
        <div className='flex flex-col lg:flex-row gap-5  items-center justify-between border-t border-gray-200 bg-white px-4 py-3 sm:px-6 mt-5'>
          <p className='text-sm text-gray-700'>
            Showing <span className='font-medium'>{indexFirst + 1}</span> to <span className='font-medium'>{currentPage === totalPages ? kategoris.length : indexLast}</span> of{" "}
            <span className='font-medium'>{kategoris.length}</span> results
          </p>
          <div className='flex flex-1 justify-between sm:hidden'>
            ...
            <button
              ...
              disabled={currentKategoris.length < page}
